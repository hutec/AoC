...
abyss = max(grid.keys(), key=lambda x: x[1])[1] + 2
...
def simulate_sand(grid) -> bool:
    ...
    while sand_next_position is None or sand_cur_position != sand_next_position:
        ...
        for dx, dy in directions:
            if (
                not grid[(sand_cur_position[0] + dx, sand_cur_position[1] + dy)]
                and sand_cur_position[1] + dy < abyss
            ):
                sand_next_position = (
    ...
    print(f"Sand fell at {sand_cur_position}")
...
while simulate_sand(grid):
    i += 1
    if grid[(500, 1)] and grid[(499, 1)] and grid[(501, 1)]:
        break
